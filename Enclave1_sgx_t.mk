...
Enclave1_Cpp_Files := $(TRUSTED_DIR)/Main.cpp 
Enclave1_C_Files := 
Enclave1_Include_Paths := -IInclude -I$(TRUSTED_DIR) -I$(SGX_SDK)/include -I$(SGX_SDK)/include/tlibc -I$(SGX_SDK)/include/libcxx
...
Enclave1_Link_Flags := $(SGX_COMMON_CFLAGS) -Wl,--no-undefined -nostdlib -nodefaultlibs -nostartfiles -L$(SGX_LIBRARY_PATH) \
	...
	-Wl,--version-script=$(TRUSTED_DIR)/Enclave1.lds
...
$(TRUSTED_DIR)/Enclave1_t.c: $(SGX_EDGER8R) ./$(TRUSTED_DIR)/Enclave1.edl
	@cd ./$(TRUSTED_DIR) && $(SGX_EDGER8R) --trusted ../$(TRUSTED_DIR)/Enclave1.edl --search-path ../$(TRUSTED_DIR) --search-path $(SGX_SDK)/include
	@echo "GEN  =>  $@"

$(TRUSTED_DIR)/Enclave1_t.o: ./$(TRUSTED_DIR)/Enclave1_t.c
...
Enclave1.so: $(TRUSTED_DIR)/Enclave1_t.o $(Enclave1_Cpp_Objects) $(Enclave1_C_Objects)
	@$(CXX) $^ -o $@ $(Enclave1_Link_Flags)
	@echo "LINK =>  $@"

Enclave1.signed.so: Enclave1.so
	@$(SGX_ENCLAVE_SIGNER) sign -key $(TRUSTED_DIR)/private.pem -enclave Enclave1.so -out $@ -config $(TRUSTED_DIR)/Enclave1.config.xml
	@echo "SIGN =>  $@"
clean:
	@rm -f Enclave1.* $(TRUSTED_DIR)/Enclave1_t.* $(Enclave1_Cpp_Objects) $(Enclave1_C_Objects)